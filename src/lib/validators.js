...
export const leadSchema = z.object({
  ...
  // Strictly requires exactly 10 digits (no letters, no symbols, no 8-digit or 12-digit numbers)
  phone: z.string().regex(/^[0-9]{10}$/, "Phone number must be exactly 10 digits."),
  
  company: z.string().optional(),
  domain: z.string().min(1, "Please select a domain."),
  
  // Strictly requires that if anything is entered, it must consist only of digits (no text like "abc")
